...
async function renderList(filter=''){
  ...
  const filtered = data.recipes.filter(r => 
    !q || r.title.toLowerCase().includes(q) || r.tags.join(' ').toLowerCase().includes(q) || r.description.toLowerCase().includes(q)
  );
  cards.innerHTML = filtered.map(card).join('');
  if(filtered.length === 0){
    cards.innerHTML = '<p>Geen resultaten. Probeer een andere zoekterm.</p>';
  }
}
...
async function loadRecipe(slug){
  const data = await getData();
  const r = data.recipes.find(x => x.slug === slug) || data.recipes[0];
  if(!r) return;
  ...
  const ing = document.getElementById('r-ingredients');
  ing.innerHTML = r.ingredients.map(i => `<li><label><input type="checkbox"> ${i}</label></li>`).join('');
